class OrderModal {
    ...
    showButtons() {
        ...
        optionsTitle.innerHTML = `Order options`;
    ...
    showModal(option) {
        ...
        switch (option) {
            case "Create": {
                modalHeader[0].innerHTML = `<h2>Create new order...</h2>`
            ...
            case "Delete": {
                modalHeader[0].innerHTML = `<h2>Delete order...</h2>`
                modalBody.innerHTML = `Code here to select and delete an event`;
                modalFooter[0].innerHTML = `<button type="button" class="btn btn-dark" data-bs-dismiss="modal" aria-label="Close">Cancel</button>`;
                break;
            }
            case "Update": {
                modalHeader[0].innerHTML = `<h2>Update order...</h2>`
